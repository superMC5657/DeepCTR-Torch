...
class self_WDL(BaseModel):
    ...
    def __init__(self,
        ...
        super(self_WDL, self).__init__(linear_feature_columns, dnn_feature_columns, l2_reg_linear=l2_reg_linear,
                                       l2_reg_embedding=l2_reg_embedding, init_std=init_std, seed=seed, task=task,
                                       device=device)
        self.use_dnn = len(dnn_feature_columns) > 0 and len(
            dnn_hidden_units) > 0 # 如果深度网络特征和深度网络隐藏层都存在
        if self.use_dnn:
